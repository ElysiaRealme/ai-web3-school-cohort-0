...
- 后续教学必须遵循 `tasks/learning-route-execution-rules.md`。
- 默认严格按本文件的 00X 顺序逐节推进。
- 当学员说“继续”或“下一节”时，进入当前目录里的下一节。
- 不跳过当前节，不一次讲多节，不擅自重排目录。
- 如果目录顺序存在问题，先反馈问题；只有学员明确说“修改目录”时才改目录。
- 如果学员说“跳到某节”或“这节我学过了”，以学员声明的进度为准，前面内容默认视为已完成。
...
状态：已跳过，复用 001-003 已掌握内容。

原因：

- 本节与 `001｜账户、地址、私钥、助记词`、`002｜签名、授权、交易`、`003｜Gas、网络、测试网、区块浏览器` 高度重复。
- 学员明确表示这些内容前面已经学过，不需要重复学习。
...
- 学员完成 1-6 题检查。
- 第 7 题因与前序 Agent / 人工确认边界问题重复，学员要求跳过，后续不再重复追问同类题。
